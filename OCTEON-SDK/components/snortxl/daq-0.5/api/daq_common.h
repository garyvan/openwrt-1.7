...
#ifdef CAV_OCT_HFA
typedef enum {
    DAQ_INJECT,              /* Inject new packet */ 
    DAQ_INJECT_REVERSE,      /* Inject new packet in reverse direction */
    DAQ_SEND                 /* Send original packet new packet */
} DAQ_Action;
#endif
...
typedef struct _daq_pkthdr
{
    ...
    DAQ_Verdict verdict;    /* Verdict used while sending the packet out */
#ifdef CAV_OCT_HFA
    union
    { 
	    void * work;               /* For OCTEON DAQ SE/SEUM  */
	    const uint8_t * raw_hdr;   /* For AFPACKET */
    }entry;
#endif
} DAQ_PktHdr_t;
