...
#ifdef CAV_OCT_SE
#define pcap_create(a,b) 0
#define pcap_lookupnet(a,b,c,d) 0
#define pcap_open_dead(a,b) 0
#define pcap_dump_open(a,b) 0
#define pcap_close(a) 0 
#define pcap_dump_close(a) 0    
#define pcap_dump(a,b,c) 0
#define pcap_freealldevs(...) 0
#define pcap_findalldevs(...) 0
#define pcap_lookupdev(a) 0
...
{
    uint32_t localnet __attribute__ ((unused));
    uint32_t netmask;
...
static void pcap_process_loop(u_char *user, const struct pcap_pkthdr *pkth, const u_char *data)
{
    ...
    DAQ_PktHdr_t* hdr = (DAQ_PktHdr_t*)func_cav128BAlloc();
		if (hdr == NULL)
		{
			printf("Unable to allocate Pkt Hdr - Out of heap memory\n");
		}
    hdr->caplen = pkth->caplen;
    hdr->pktlen = pkth->len;
    hdr->device_index = -1;
    hdr->flags = 0;
    hdr->ts.tv_sec = pkth->ts.tv_sec;
    hdr->ts.tv_usec = pkth->ts.tv_usec;
    ...
    context->analysis_func(context->user_data, hdr, data);
#ifndef CAV_OCT_HFA
