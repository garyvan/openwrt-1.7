...
#ifdef CAV_OCT_HFA
//This indicates that the entry is being used by software
//Is it wise to modify a flag that will be used by the KERNEL?
//This bit is currently unused.
#define TP_STATUS_CAV 1 << 28

extern void (*func_processCtx_lin)(int); 
void* (*func_cav128BAlloc)(void);
void (*func_cav128BFree)(void*);
#endif
...
/* Cavium:
 * Inject Routines can be called for injecting new packets in case of active
 * responses/normalizations.
 * However we use the same interface to send out packets in case of async
 * packets.
 *
 * The DAQ_SEND macro is used for this purpose:
 *
 */ 
static int afpacket_daq_inject(void *handle, const DAQ_PktHdr_t *hdr, const uint8_t *packet_data, uint32_t len, int reverse)
{
    ...
    if (!instance || (!(action == DAQ_INJECT_REVERSE ) && !(instance = instance->peer)))
        return DAQ_ERROR;
    ...
    if (action == DAQ_SEND )
    {
        ...
        /* This will also set the doftware bit to zero */
        t_hdr.raw = hdr->entry.raw_hdr;
        switch (instance->tp_version)
        {
            case TPACKET_V1:
                t_hdr.h1->tp_status = TP_STATUS_KERNEL;
                break;
#ifdef HAVE_TPACKET2
            case TPACKET_V2:
                t_hdr.h2->tp_status = TP_STATUS_KERNEL;
                break;
#endif
        }
    ...
    while (cnt <= 0 || c < cnt)
    {
        ...
        for (instance = afpc->instances; instance; instance = instance->next)
        {
               ...
               )
            {
                ...
                if ((hdr.h1->tp_status & TP_STATUS_CAV)
#ifdef HAVE_TPACKET2
                    ||(hdr.h2->tp_status & TP_STATUS_CAV)
#endif
                   )
                {
                  /* Here all ring entries are pending with SnortXL - 
                   * SO complete them and the go to get new packets 
                   * This should not happen in any normal case
                   * */
                  printf("All buffers are pending with SnortXL - Anomalous \n");
                  
                  (*func_processCtx_lin)(1); /* Process all Ctx for pending sbufs */ 
                  break;
                }
                ...
                switch (instance->tp_version)
                {
                    case TPACKET_V1:
                        ...
                        hdr.h1->tp_status |= TP_STATUS_CAV;
                        break;
#ifdef HAVE_TPACKET2
                    case TPACKET_V2:
                ...
                daqhdr = (DAQ_PktHdr_t *)func_cav128BAlloc();
                if(daqhdr == NULL)
                {
                  printf("Out of heap mem for DAQ hdr\n");
                  /* SnortXL: Verify return values */
                    return 0;
                }
                ...
                daqhdr->flags = 0;
                daqhdr->entry.raw_hdr = instance->entry->hdr.raw; 
                verdict = DAQ_VERDICT_PASS;
                if (afpc->fcode.bf_insns && sfbpf_filter(afpc->fcode.bf_insns, data, tp_len, tp_snaplen) == 0)
                {
                    ignored_one = 1;
                    afpc->stats.packets_filtered++;
                    afpacket_daq_inject(handle, daqhdr, data, tp_snaplen, 0);
                    goto post_send;
                }
                got_one = 1;

#ifndef CAV_BYPASS //ABD: define just to measure socket perf without any snort overhead
                if (callback)
                {
...
#ifdef CAV_BYPASS
                afpacket_daq_inject(handle, daqhdr, data, tp_snaplen, 2);
#endif
post_send:
        ...
        if (!got_one && !ignored_one)
        {
            ...
            (*func_processCtx_lin)(1); /* Process all Ctx for pending sbufs after polling for new input packets */ 
