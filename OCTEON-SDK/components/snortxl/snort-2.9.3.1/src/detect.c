...
#ifdef CAV_OCT_SE
	    p->snort_preproc_cycle = cvmx_get_cycle();
#endif
...
#ifdef CAV_OCT_ASYNC
	/* No OMDs will be associated to this packet after this point */
    p->state_flag = CAV_STATE_FINAL;
#endif

#if 000 
	/* SnortXL ppm manager needs to be fixed - with src release 
	 *  Abed: Leave this code till all are properly handled
	 *  Events are being handled in the cavOctFree API 
	 * */
#ifndef CAV_OCT_HFA_GCOMPILE
	if (cvmx_unlikely(cav_search_method != CAV_OCT_HFA_ENGINE) || search_type == SYNC)
	{
    	/* Being done in matchcb for HFA case */	
...
#ifdef CAV_OCT_HFA
	if(search_type == SYNC)
		account_cycles(p->snort_preproc_cycle, SNORT_PREPROC_CYCLES);
#endif
#endif
    return retval;
}
...
#ifdef CAV_OCT_SE
    p->detect_start_cycle = cvmx_get_cycle();
#endif
...
#ifdef CAV_OCT_HFA
	if(search_type == SYNC)
		account_cycles(p->detect_start_cycle, SNORT_DETECT_CYCLES);
#endif
    return detected;
}
