...
#define DETECTION_OPT__MAX_HFA_LEN                           "max-hfa-len"
#define DETECTION_OPT__MIN_HFA_LEN                           "min-hfa-len"
#define DETECTION_OPT__HFA_PIPELINE_DEPTH                    "hfa-pipeline-depth"
#endif
...
#else
uint32_t hfa_pipeline_depth;
#endif
#endif

#if defined(CAV_OCT_HFA)
#include "exec_intf.h"
#endif
...
static void ParseGraph(SnortConfig *, SnortPolicy *, char *);
...
static const KeywordFunc snort_conf_keywords[] =
{
    ...
    { SNORT_CONF_KEYWORD__GRAPH,             KEYWORD_TYPE__MAIN, 0, 0, ParseGraph },
...
#ifdef CAV_OCT_HFA
    /* port toggling support using -a flag */
    { CONFIG_OPT__PORT_TOGGLE, 1, 1, 1, ConfigPort },
    { CONFIG_OPT__DUAL_BOOT, 1, 1, 1, ConfigDualBoot },
#endif
...
SnortConfig * ParseSnortConf(void)
{
    ...
    sc->omd->matchInfo = (MATCH_INFO *)SnortAlloc(sizeof(MATCH_INFO)*8); 
...
#ifndef CAV_OCT_SE
                        /*Cavium: stat is not supported in SE */
...
#ifndef CAV_OCT_SE
                        /* do a quick check to make sure file exists */
                        /* file format is not supported in SE */
...
static void ParseGraph(SnortConfig *sc, SnortPolicy *p, char *arg)
{
#ifndef CAV_OCT_SE
    int path_len = strlen(snort_conf_dir) + strlen(arg) + 1;
    snprintf(graphpath, path_len, "%s%s", snort_conf_dir, arg);
#else
    int path_len = strlen(arg) + 1;
    snprintf(graphpath, path_len, "%s", arg);
#endif
}
...
void ConfigDetection(SnortConfig *sc, char *args)
{
    ...
    for (i = 0; i < num_toks; i++)
    {
...
#if( defined(CAV_OCT_HFA) || defined(CAV_OCT_HFA_GCOMPILE))
        else if (strcasecmp(toks[i], DETECTION_OPT__HFA_PIPELINE_DEPTH) == 0)
        {
            i++;
            if (i < num_toks)
            {
                ...
                if ((errno == ERANGE) || (*endptr != '\0') || (n < 1) || (n > 20))
                {
                    ParseError("Invalid argument for hfa-pipeline-depth: %s.  "
                               "Valid Range 1-20, Recommended 4.", toks[i]);
                }

                hfa_pipeline_depth = n;
            }
            else
            {
                ParseError("Missing argument to 'max-hfa-len'.");
            }
        }
        else if (strcasecmp(toks[i], DETECTION_OPT__MAX_HFA_LEN) == 0)
        {
            i++;
            if (i < num_toks)
            {
                ...
                if ((errno == ERANGE) || (*endptr != '\0') || (n < 0))
                {
                    ParseError("Invalid argument for max-hfa-len: %s.  "
                               "Need a non-negative integer.", toks[i]);
                }
        ...
        else if (strcasecmp(toks[i], DETECTION_OPT__MIN_HFA_LEN) == 0)
        {
            i++;
            if (i < num_toks)
            {
                ...
                if ((errno == ERANGE) || (*endptr != '\0') || (n < 0))
                {
                    ParseError("Invalid argument for min-hfa-len: %s.  "
                               "Need a non-negative integer.", toks[i]);
                }
            ...
            {
                ParseError("Missing argument to 'min-hfa-len'.");
            }
        }
        /* support for graph-merge on OCTEON_SE and x86*/
        else if (strcasecmp(toks[i], NO_MERGE_GRAPHS) == 0)
        {
            merge_graphs = 0;
        }
#endif
...
#if (defined CAV_OCT_LINUX) || (defined CAV_OCT_HFA_GCOMPILE)
        /* support for graph-generation on OCTEON_LINUX and x86 */
        else if (strcasecmp(toks[i], CREATE_GRAPHS) == 0)
        {
            create_graphs=1;
        }
        else if (strcasecmp(toks[i], GRAPH_PATH) == 0)
        {
            i++;
            if (i < num_toks)
                strcpy(graphpath,toks[i]);
            else
            {
                printf("\nMissing argument to graphpath using default one i.e. /home/graphs\n");
                strcpy(graphpath,"/home/graphs/");
            }
        }
#endif
        else
...
#ifdef CAV_OCT_HFA
/* support for port toggling in SE */
extern CVMX_SHARED int used_port_sets;
extern CVMX_SHARED unsigned long int port_set[MAX_PORT_SETS][2];
CVMX_SHARED unsigned int sf_cmask = 0;
void ConfigPort(SnortConfig *sc, char *args)
{
    char *endptr;
    char *saveptr;
    char *tmp_str;
    ...
    tstr = strdup(args);
    if(tstr == NULL)
        FatalError("strudp failed\n");
    
	tmp_str = strtok_r(tstr,":",&saveptr);
    port1 = SnortStrtoul(tmp_str,&endptr,0);
    if ((errno == ERANGE) || (*endptr != '\0'))
    {
        ParseError("Invalid ports:%s ", args);
    }

    tmp_str = strtok_r(NULL,":",&saveptr);
    port2 = SnortStrtoul(tmp_str,&endptr,0);
    if ((errno == ERANGE) || (*endptr != '\0'))
    {
        ParseError("Invalid ports:%s ", args);
    }
    ...
    {
        ParseError("Ran out of port sets");
        return;
    }
    port_set[used_port_sets][0] = port1;
    port_set[used_port_sets][1] = port2;
    LogMessage("Toggling Ports are : %lu,%lu\n",port_set[used_port_sets][0],port_set[used_port_sets][1]);
    used_port_sets ++;

}

void ConfigDualBoot(SnortConfig *sc, char *args)
{
    char *endptr = NULL;
    char *saveptr = NULL;
    ...
    if(tstr == NULL)
        FatalError("strudp failed\n");
    tmp_str1 = strtok_r(tstr,",",&saveptr);
    if (tmp_str1 == NULL)
        ParseError("Invalid Config File");
    tmp_str2 = strtok_r(NULL,",",&saveptr);
    sf_cmask = (unsigned int)SnortStrtoul(tmp_str2,&endptr,0);
    if ((errno == ERANGE) || (*endptr != '\0') ||
#ifdef SDK_3_1
        (((cav_oct_get_coremask()) & sf_cmask) != sf_cmask) ||
#else
        (((cvmx_sysinfo_get()->core_mask) & sf_cmask) != sf_cmask) || 
#endif
        ((sf_cmask & 0x1) != 0))
    {
        ParseError("Invalid Sf coremask:%x ", sf_cmask);
    }
#ifdef SDK_3_1
    if(cav_oct_coremask_is_member(sf_cmask)){
#else
    if(cvmx_coremask_is_member(sf_cmask)){
#endif
        sc->run_mode_flags |= RUN_MODE_FLAG__IDS;
        snort_conf_file = strdup(tmp_str1);
#ifdef SDK_3_1
        if(cav_oct_is_first_core_from_mask(sf_cmask))
#else
        if(cvmx_coremask_first_core(sf_cmask))
#endif
        printf("Dual Boot set\n");
    }
}
#endif
...
#ifndef CAV_OCT_HFA_GCOMPILE
    /* remove references to routines in libpcap */
    else
    {
