...
#else
#ifdef CAV_OCT_HFA
// Cavium: support SE specific DAQ module
#define DAQ_DEFAULT "octeon"
#else
...
#ifdef CAV_OCT_HFA
int cavOctDaqFlag = 0;
int cavOctPcapFlag = 0;
extern void (*func_pStats)(void);
extern void (*func_show_preproc_profile)(void);
void pStats(void)
{
    TimeStop();
    DropStats(0);
...
void DAQ_Init (const SnortConfig* sc)
{
    ...
    /* CAVIUM: only Daq pcap, octeon and afpacket are supported as of Now */

    if(!((strcmp(type,"octeon") ==0) || (strcmp(type,"pcap") == 0) || (strcmp(type,"afpacket") == 0)))
        FatalError("Only DAQ pcap, octeon and afpacket are supported\n");

    if(!strcmp(type,"pcap")) 
        cavOctPcapFlag = 1;

    if(!strcmp(type,"octeon")) {
        cavOctDaqFlag = 1;

        if(InitPortSets(sc->interface) < 0)                    //initialize Port Sets Here for octeon DAQ
            FatalError("Port Initialization Failed \n");

        func_pStats = pStats;
#ifdef PERF_PROFILING    
        func_show_preproc_profile=ShowPreprocProfiles;
#endif
    }
#endif
...
#else
        // Cavium: support SE specific DAQ module
        //LogMessage("Acquiring network traffic.\n");
#endif
    }
