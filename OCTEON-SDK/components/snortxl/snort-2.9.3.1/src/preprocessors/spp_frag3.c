...
CVMX_SHARED static Frag3Stats f3s[CVMX_MAX_CORES+1];
#endif
...
static void Frag3PrintStats(int exiting)
{
    ...
    // Cavium: support for single core stats printing 
    int i=0;
    f3s[cav_oct_core] = f3stats;
    
    cavbarrier_sync();
    if (cavlast_core())
    {  
       memset(&f3s[CVMX_MAX_CORES],0,sizeof(Frag3Stats));
       for(i=0;i<CVMX_MAX_CORES;i++)
       {
          f3s[CVMX_MAX_CORES].total                     += f3s[i].total;
          f3s[CVMX_MAX_CORES].reassembles               += f3s[i].reassembles;
          f3s[CVMX_MAX_CORES].discards                  += f3s[i].discards;
          f3s[CVMX_MAX_CORES].prunes                    += f3s[i].prunes;
          f3s[CVMX_MAX_CORES].timeouts                  += f3s[i].timeouts;
          f3s[CVMX_MAX_CORES].overlaps                  += f3s[i].overlaps;
          f3s[CVMX_MAX_CORES].anomalies                 += f3s[i].anomalies;
          f3s[CVMX_MAX_CORES].alerts                    += f3s[i].alerts;
          f3s[CVMX_MAX_CORES].drops                     += f3s[i].drops;
          f3s[CVMX_MAX_CORES].fragtrackers_created      += f3s[i].fragtrackers_created;
          f3s[CVMX_MAX_CORES].fragtrackers_released     += f3s[i].fragtrackers_released;
          f3s[CVMX_MAX_CORES].fragtrackers_autoreleased += f3s[i].fragtrackers_autoreleased;
          f3s[CVMX_MAX_CORES].fragnodes_created         += f3s[i].fragnodes_created;
          f3s[CVMX_MAX_CORES].fragnodes_released        += f3s[i].fragnodes_released;
       }
#define f3stats f3s[CVMX_MAX_CORES]
#endif
