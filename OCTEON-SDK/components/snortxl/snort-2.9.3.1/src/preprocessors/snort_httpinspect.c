...
/* SNORTXL:uri_used keeps track of HttpUriBuffers */
int uri_used = 0;
...
int SnortHttpInspect(HTTPINSPECT_GLOBAL_CONF *GlobalConf, Packet *p)
{
    ...
    */
    do
    {
        ...
        */
#ifdef CAV_OCT_HFA 
        /* SNORTXL:If URI is occupied by any pending search in pipeline
        ** then poll for search completion before setting URI buffers to zero
        */
        if( uri_used )
        {
            cavOctSbufPoll(0);
        ...
        */
        iCallDetect = 0;
        /* SNORTXL:Poll Pending searches for completion and postprocess,reset
        ** matchinfo before overwriting matchinfo and omd_count while processing 
        ** next pipelined request.
        */
#ifdef CAV_OCT_HFA 
        MATCH_INFO *matchInfo = (MATCH_INFO *)p->matchInfo;
        int i = 0;
        if( (p->state_flag == CAV_STATE_SUBMIT) && Session->client.request.pipeline_req )
        {
            p->state_flag = CAV_STATE_PIPELINE;
            cavProcessPkt(0);
            cavOctSbufPoll(1);
...
#ifdef CAV_OCT_HFA 
    if( ( p->state_flag != CAV_STATE_SUBMIT ) && (p->uri_count) )
        uri_used = 0;
#endif
    if ( iCallDetect == 0 )
    {
