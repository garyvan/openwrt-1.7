...
#endif
#ifdef CAV_OCT_HFA
extern CVMX_SHARED cvmx_arena_list_t       cav_arena;
extern CVMX_SHARED cvmx_spinlock_t         cav_arena_lock;
extern CVMX_SHARED void*                   cav_arena_addr ;
#endif
...
int Stream5VerifyTcpConfig(Stream5TcpConfig *config, tSfPolicyId policy_id)
{
     ...
     */
#ifndef CAV_OCT_HFA
    /* Stream5 needs not to be initialized here as its done every time before getting processed */
    if (!s5_pkt)
        Stream5InitPacket();
#endif
...
void Stream5CleanTcp(void)
{
    ...
    /* Stream5 needs not to be cleaned here as its done every time after getting flushed */
#ifndef CAV_OCT_HFA
...
static inline int purge_to_seq(TcpSession *tcpssn, StreamTracker *st, uint32_t flush_seq)
{
    ...
    /* SNORTXL:Dont flush number of alerts since packets associated with 
     * flushed segment may be still pending for search */
...
/* Call Free function for Stream5 Packet before returning from flush_to_seq */
static inline void cavStream5Free(void)
{
#ifdef CAV_OCT_ASYNC
    s5_pkt->state_flag = CAV_STATE_FINAL;
    cavOctFree(s5_pkt);
#endif
}
    ...
    snort_ip_p sip, snort_ip_p dip, uint16_t sp, uint16_t dp, uint32_t dir)
{
    ...
    //PrintSeglist(st);
#ifdef CAV_OCT_ASYNC
    /* Allocate s5 packet before calling respective flush_to_seq */
    s5_pkt = (Packet *)cavOctAlloc( );
    if (s5_pkt == NULL)
    {
        printf("Unable to allocate S5 Pkt pool buffer FPA Pool \n");
        return -1;
    }
    s5_pkt->pkt = cav_arena_alloc( 65536 ,CVMX_CACHE_LINE_SIZE);
    if(s5_pkt->pkt == NULL)
    {
        printf("Unable to allocate 8k buffer from arena \n");
        return -1;
    }
    s5_pkt->pkth = (DAQ_PktHdr_t*)func_cav128BAlloc( );
#endif
    ...
    /* Set final flag for stream5 packet as there cant be anymore associated
     * search after this point */
    if ( st->tcp_policy )
        UpdateFlushMgr(&st->flush_mgr, &st->tcp_policy->flush_point_list);
...
/* Cavium: Releases the stream and Cleans the Tcp Session if flush_flags is set */
void cavSessionCleanup( Stream5LWSession *lwssn)
{
    TcpSession *tcpssn = NULL;
    ...
    if(lwssn->flush_flags & CAV_FLUSH_SSNCLEANCLIENT)
        purge_ackd(tcpssn,&tcpssn->client);

    if(lwssn->flush_flags & CAV_FLUSH_SSNCLEANSERVER)
        purge_ackd(tcpssn,&tcpssn->server);

    if(lwssn->flush_flags & CAV_FLUSH_OVERLAP)
    {
        purge_all(&tcpssn->client);
        purge_all(&tcpssn->server);
    }
...
void TcpSessionCleanup(Stream5LWSession *lwssn)
{
    ...
    /* Flush ack'd data on both sides as necessary */
    {
        ...
        if (tcpssn->client.seglist && !(lwssn->ignore_direction & SSN_DIR_SERVER) )
        {
            ...
            if (flushed)
                    lwssn->flush_flags |= CAV_FLUSH_SSNCLEANCLIENT;
        ...
        if (tcpssn->server.seglist && !(lwssn->ignore_direction & SSN_DIR_CLIENT) )
        {
            ...
            if (flushed)
                    lwssn->flush_flags |= CAV_FLUSH_SSNCLEANSERVER;
  ...
  /* Process pomdlist before exiting in case of stream5 cleanup */ 
    cavProcessPkt(NULL);
 
  /* Poll For completion of pending search before clearing the session */
    cavOctSbufPoll(1);

    if ( lwssn->flush_flags & CAV_FLUSH_ALL )
    ...
    if (lwssn == NULL)
    {
        ...
        if (s5TcpPolicy->flags & STREAM5_CONFIG_REQUIRE_3WHS)
        {
                ...
                !TCP_ISFLAGSET(p->tcph, TH_ACK))
            {
                ...
                lwssn->pending = 0;
                lwssn->flush_flags = CAV_FLUSH_NONE;
            }
            else
            {
    ...
    if(rcv->seg_count != 0)
    {
        ...
        else
        {
                ...
                (rcv->overlap_count > rcv->tcp_policy->overlap_limit))
            {
                ...
                tcpssn->lwssn->flush_flags |= CAV_FLUSH_OVERLAP;
...
extern uint8_t cav_bypass;
extern uint8_t bypass_threshold;
        ...
        Stream5TcpPolicy *s5TcpPolicy)
{
    ...
    int aligned __attribute__ ((unused)) = S5_UNALIGNED;
    TcpSession *tcpssn = NULL;
    StreamTracker *talker = NULL;
    StreamTracker *listener = NULL;
    ...
    if (lwssn->proto_specific_data)
    {
        ...
        if (cav_bypass)
        {
           if (tcpssn && bypass_threshold)
           {
              tcpssn->packets_tcp_seen ++;
              if (tcpssn->packets_tcp_seen > bypass_threshold)
              {
	   		  Stream5StopInspection(lwssn, p, SSN_DIR_BOTH, -1, 0);
              }
           }
        }
    }
...
//dropssn:
        ...
        if(p->packet_flags & PKT_FROM_SERVER)
        {
            ...
            if(talker->seg_bytes_logical)
            {
                ...
                if(flushed) /*Flush is not handled with Async here*/
                {
        ...
        /* yoink that shit */
        LogTcpEvents(listener->tcp_policy, eventcode);
        /* Set packet state to final before deleting the session */
#ifdef CAV_OCT_ASYNC
