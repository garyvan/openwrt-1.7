...
#include "misc_defs.h"
CVMX_SHARED static HIStats hStats[CVMX_MAX_CORES+1];
#endif
...
// Cavium: to prevent compiler warnings 
#ifdef CAV_OCT_HFA
...
{
#ifdef CAV_OCT_HFA /* defined CAV_OCT_HFA */
    hStats[cav_oct_core] = hi_stats;   		//SNORTXL:: collect stats from all cores
    cavbarrier_sync();
    ...
    if (cavlast_core())
    {
       memset(&hStats[CVMX_MAX_CORES],0,sizeof(HIStats));
       for(i=0;i<CVMX_MAX_CORES;i++)
       {
           hStats[CVMX_MAX_CORES].unicode            += hStats[i].unicode;
           hStats[CVMX_MAX_CORES].double_unicode     += hStats[i].double_unicode;
           hStats[CVMX_MAX_CORES].non_ascii          += hStats[i].non_ascii;
           hStats[CVMX_MAX_CORES].dir_trav           += hStats[i].dir_trav;
           hStats[CVMX_MAX_CORES].slashes            += hStats[i].slashes;
           hStats[CVMX_MAX_CORES].self_ref           += hStats[i].self_ref;
           hStats[CVMX_MAX_CORES].post               += hStats[i].post;
           hStats[CVMX_MAX_CORES].get                += hStats[i].get;
           hStats[CVMX_MAX_CORES].post_params        += hStats[i].post_params;
           hStats[CVMX_MAX_CORES].req_headers        += hStats[i].req_headers;
           hStats[CVMX_MAX_CORES].resp_headers       += hStats[i].resp_headers;
#ifdef DEBUG                                                      
           hStats[CVMX_MAX_CORES].req_header_len     += hStats[i].req_header_len;
           hStats[CVMX_MAX_CORES].resp_header_len    += hStats[i].resp_header_len;
#endif                                                            
           hStats[CVMX_MAX_CORES].req_cookies        += hStats[i].req_cookies;
           hStats[CVMX_MAX_CORES].resp_cookies       += hStats[i].resp_cookies;
#ifdef DEBUG                                                      
           hStats[CVMX_MAX_CORES].req_cookie_len     += hStats[i].req_cookie_len;
           hStats[CVMX_MAX_CORES].resp_cookie_len    += hStats[i].resp_cookie_len;
#endif                                                            
           hStats[CVMX_MAX_CORES].total              += hStats[i].total;
#ifdef ZLIB                                                       
           hStats[CVMX_MAX_CORES].gzip_pkts          += hStats[i].gzip_pkts;
           hStats[CVMX_MAX_CORES].compr_bytes_read   += hStats[i].compr_bytes_read;
           hStats[CVMX_MAX_CORES].decompr_bytes_read += hStats[i].decompr_bytes_read;
#endif
       }
#define hi_stats  hStats[CVMX_MAX_CORES]
    if (!hi_stats.total)           //SNORTXL:: exit if no HttpInspect stats 
        return;
#endif 
