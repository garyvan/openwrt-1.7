...
#ifdef CAV_OCT_LINUX
#include <pthread.h>
extern volatile int cav_breakloop;
#endif

#ifdef CAV_OCT_HFA
// Cavium: to include cavium specific definitions
#include "sfutil/cav_oct_hfa.h"
#include "misc_defs.h"
#ifdef CAV_OCT_SE
/* SE specific unistd.h as generic unistd.h is not completely supported */
#include <custom/unistd.h>
#endif
extern int cav_cli;
extern int rcv_cores;
extern int profile;
extern uint32_t hfa_pipeline_depth;
extern int cav_profile;
#endif
...
extern int cacheLoads, cacheEvictions;
...
#if !defined(WIN32) && !defined(CAV_OCT_SE)
/* pthread is not supported in SE */
...
// Cavium: new flag to get stats only once when --pcap-loop is used 
static int pcap_showtotalstats = 0;
...
#ifdef CAV_OCT_HFA
/* needed to be shared across cores */
CVMX_SHARED static volatile int usr_signal = 0;
#else
static volatile int usr_signal = 0;
#endif 
...
#ifdef CAV_OCT_HFA
/* port toggling support in SE using -a flag */
    "?a:A:bB:c:CdDeEfF:"
#else
    "?A:bB:c:CdDeEfF:"
#endif
...
static struct option long_options[] =
{
   ...
   // Cavium: new flag to get stats only once when --pcap-loop is used 
   {"pcap-total-stats", LONGOPT_ARG_NONE, NULL, PCAP_TOTALSTATS},
   ...
   /* Use this option to bypass stream after a number of pkts (defined by the 
    * argument for this option) have been inspected */
   {"enable-bypass", LONGOPT_ARG_REQUIRED, NULL, ENABLE_BYPASS},

   {"cs-dir", LONGOPT_ARG_REQUIRED, NULL, ARG_CS_DIR},

#ifdef CAV_OCT_SE
   {"cli-enable", LONGOPT_ARG_NONE, NULL, CAV_OCT_CLI},
#endif
   {"rcv-cores", LONGOPT_ARG_REQUIRED, NULL, CAV_OCT_RCV},
   {"hfa-pipeline-depth", LONGOPT_ARG_REQUIRED, NULL, CAV_OCT_HFA_PL_DEPTH},
   {"profile", LONGOPT_ARG_NONE, NULL, CAV_OCT_PROFILE},
   {"cav_profile", LONGOPT_ARG_NONE, NULL, CAV_OCT_CAV_PROFILE},
 ...
 */
#ifdef CAV_OCT_LINUX
// Cavium: support for octeon linux 
...
int SnortMain(int argc, char *argv[])
{
    ...
    CavRemoteLogInit(inet_addr(snort_conf->syslog_server),snort_conf->syslog_server_port);
#endif
...
#else
#ifdef CAV_OCT_SE
        // Cavium: equivalent sleep fn in SE
        cav_wait(1500);
#else
        sleep(1);
#endif
...
{
    /* Cavium: the packet 'p' memory allocation has been moved from stack(local variable)
     * to heap(pointer) so that HFA can access the pointer in a match callback.
     * References to 'p' in the code below have also been changed accordingly.
     */
    Packet *p = NULL;
    int retval=0;
    DAQ_PktHdr_t* daqhdr = (DAQ_PktHdr_t*) pkthdr;  
    daqhdr->verdict = DAQ_VERDICT_PASS;
    PROFILE_VARS;

#ifdef DUMP_PKT_STATS
extern int global_pkt_num;
#endif

#ifdef CAV_OCT_SE
    if (cvmx_unlikely(pkthdr->pktlen >= OCTEON_IBUFPOOL_SIZE))
    {
        /* We are allowing jumbo pkts to pass untouched and unanalyzed !*/ 
        return daqhdr->verdict;
    }
    account_stats(pkthdr->pktlen, SNORT_LENGTH);
#endif

#ifdef CAV_OCT_HFA
    p = (Packet *)cavOctAlloc();
#else
...
#ifdef CAV_OCT_SE
        p->snort_start_cycle = cvmx_get_cycle();
        p->snort_diff_cycle = 0;
#endif
    ...
    pc.total_from_daq++;
  //  if (!(pc.total_from_daq % 10000)) printf("[C%d]Packets %d\n",cvmx_get_core_num(), pc.total_from_daq);
#ifdef DUMP_PKT_STATS
    global_pkt_num = pc.total_from_daq;
#endif
    ...
    if (ScTerminateService() || ScPauseService())
    {
        return daqhdr->verdict;  // time to go
    }
#endif
    ...
    retval = ProcessPacket(p, pkthdr, pkt, NULL);
...
#ifdef CAV_OCT_SE
    if(search_type == SYNC)
        account_cycles(p->snort_start_cycle, SNORT_TOTAL_CYCLES);
#endif
#ifdef CAV_OCT_HFA
    /* Assuming Packet didn't go in lets free it here */
#ifdef CAV_OCT_ASYNC
    p->state_flag = CAV_STATE_FINAL;
#endif
    /* Packet will be freed here only if there are no HFA submits on this -
     * otherwise it will be freed after postprocess 
     */    
    cavProcessPkt(p);
#endif
    return retval;
...
#ifdef CAV_OCT_SE
    p->process_cycles = cvmx_get_cycle();
#endif
...
#ifdef CAV_OCT_SE
    account_cycles(p->process_cycles, SNORT_PROC_CYCLES);
#endif
    return 0;
}
...
# define FPUTS_BOTH(msg)  fputs(msg,stdout)
    ...
    FPUTS_BOTH ("        -i <if>    Listen on interface <if>\n"
                "           <X:Y>   In DAQ OCTEON mode traffic toggles between ports X:Y  \n"
                "                   If no port is specified traffic will be sent back on same port it has been recieved\n");
    ...
    FPUTS_BOTH ("   --pcap-total-stats              special parameter only applicable when used with 'pcap-loop' where stats are printed only once.\n");
...
uint8_t cav_bypass=0;
uint8_t bypass_threshold=0;
/*
...
#if !defined(CAV_OCT_SE) && !defined(CAV_OCT_HFA_GCOMPILE)
                /* logs are not supported in SE */
...
#ifdef CAV_OCT_HFA
    /* support for single core stats printing */
    if (!cavlast_core()){
        /* All others cores need to be silent */
        ConfigQuiet(sc, NULL);
    }
#endif
    ...
    */
...
#ifdef CAV_OCT_SE
    /* support for -i switch in command line */
    if(sc->interface == NULL)
    {
        sc->interface = SnortAlloc(5);
        strncpy(sc->interface,"eth0",5);
    }
#endif
    ...
    while ((ch = getopt_long(argc, argv, valid_options, long_options, &option_index)) != -1)
    {
        ...
        switch (ch)
        {
            ...
            case 'i':
                ConfigInterface(sc, optarg); //Cavium : Internally ports are parsed here itsef port1:port2 for inline mode
                //ConfigPort(sc, optarg);
                break;
            ...
            case 'J':  /* use configuration file x for first core*/
                ConfigDualBoot(sc, optarg);
                break;
#endif
            ...
            case ENABLE_BYPASS:
                {
                   char *endptr;
                   LogMessage("Enable Bypass Mode\n");
                   bypass_threshold = SnortStrtoul(optarg, &endptr, 0);
                   cav_bypass=1;
                   printf("Enabling bypass after %d packets\n", bypass_threshold);
                }
                break;
...
#if !(defined WIN32 || defined CAV_OCT_SE)
                /* Cavium: SE doesn't support remote logging */
                ParseOutput(sc, NULL, "alert_syslog");
#else
                sc->logging_flags |= LOGGING_FLAG__SYSLOG_REMOTE;
#endif
            ...
            case PCAP_TOTALSTATS:
                // Cavium: new flag to get stats only once when --pcap-loop is used 
                pcap_showtotalstats=1;
                break;
            ...
            case CAV_OCT_RCV:
                rcv_cores = atoi(optarg);
                break;
            case CAV_OCT_HFA_PL_DEPTH:
                hfa_pipeline_depth = atoi(optarg);
                break;
            case CAV_OCT_PROFILE:
                profile = 1;
                break;
            case CAV_OCT_CAV_PROFILE:
                cav_profile = 1;
                break;
#endif
            case '?':  /* show help and exit with 1 */
    ...
    // Cavium: new flag to get stats only once when --pcap-loop is used 
    if (pcap_showtotalstats && !pcap_loop_count)
    {
        FatalError("--pcap-total-stats can only be used in combination with "
                   "--pcap-loop on the command line.\n");
    }
...
static void SigExitHandler(int signal)
{
    ...
    {
#ifdef CAV_OCT_HFA
        // Cavium: Cavium specific exit routine 
        cavOctHfaExit();
#endif
        _exit(0);
    }
...
/* SNORTXL:Called after preprocessor exits since pending packets get injected there */
/*    if ( DAQ_WasStarted() )
    {
#ifdef EXIT_CHECK
...
static void SnortInit(int argc, char **argv)
{
    InitSignals();
#if defined(NOCOREFILE) && !defined(WIN32)
    SetNoCores();        /* NOT Compiled for OCT SE */
#endif
...
#ifdef CAV_OCT_HFA
    /* initializing hfa components */
    cav_user_app_init ();
    //cavOctHfaInit();
#ifdef CAV_OCT_SE
    if(init_named_block())
        ERR("init_named_block() failed\n");
#endif
#endif
