...
// Cavium: Add HFA search engine support
...
#if defined CAV_OCT_HFA || defined CAV_OCT_HFA_GCOMPILE 
        // Cavium: Add HFA search engine support
        case MPSE_CAV_OCT_HFA:
            p->obj=cavOctHfaNew(userfree, optiontreefree, neg_list_free);
            break;
#endif
        default:
...
#ifdef CAV_OCT_HFA
        //  SnortXL Cavium: HFA graphs delete
        case MPSE_CAV_OCT_HFA:
            if (p->obj)
                cavOctHfaDelete((CAV_OCT_HFA_MPSE *)p->obj);
            free(p);
            break;
#endif
...
#if defined CAV_OCT_HFA || defined CAV_OCT_HFA_GCOMPILE 
     /* 
	  * SnortXL Cavium : Entry point for pattern addition for HFA graphs - see
	  * if other HFA features can be utilized here 
	  *
	  * In some cases where content is not unique enough and there are pcre,
	  * consider the possibility of using pcre as fast pattern also - can be 
	  * added to the same graph and will help reduce fast matches. 
	  */
     case MPSE_CAV_OCT_HFA:
       return cavOctHfaAddPattern((CAV_OCT_HFA_MPSE *)p->obj, (unsigned char *)P, m,
               noCase, negative, ID, IID);
#endif
     default:
       return -1;
...
#if defined CAV_OCT_HFA || defined CAV_OCT_HFA_GCOMPILE 
     /* 
	  * SnortXL Cavium : Entry point for Graph compilation - Internally we 
	  * decide importance of graphs and allocate hardware resources 
	  * accordingly
	  */
     case MPSE_CAV_OCT_HFA:
       return cavOctHfaCompile((CAV_OCT_HFA_MPSE *)p->obj, build_tree, neg_list_func);
#endif
...
#ifdef DUMP_PKT_STATS
#ifdef DUMP_PKT_DATA
    unsigned char *tmp = T;int i;static int j = 1;
    printf("\n");
    printf("----------------PKT: %d --DATA-- SIZE: %d---------------------------\n", global_pkt_num, n);
    for(i=0;i<n;i++)
    {
       if((i+1)%16 == 0)
           printf("%02x\n",*tmp++);
...
#ifdef CAV_OCT_HFA
     /* SnortXL Cavium: Entry point for HFA search
	  * Most of the buffers that are passed from httpinspect are stack variables
	  * and will not persist during post process in asynchronous case - 
	  * therefore this api will internally maintain a copy of the search buffer
	  * which will be freed upon search completion.
	  *
	  * The Packet, OMD are also preserved and are freed after results are 
	  * available from the engine and post processing is complete.
	  * 
	  */
     case MPSE_CAV_OCT_HFA:
        
        ret = cavOctHfaSearch((CAV_OCT_HFA_MPSE *)p->obj, (unsigned char *)T, n, action, data);

        *current_state = 0;
        PREPROC_PROFILE_END(mpsePerfStats);
        if(ret)
            FatalError("cavOctHfaSearch failed\n");
        return ret;
#endif
     default:
       PREPROC_PROFILE_END(mpsePerfStats);
...
int mpseGetPatternCount(void *pvoid)
{
    ...
    switch( p->method )
    {
        ...
        // Cavium: Add HFA search engine support
        case MPSE_CAV_OCT_HFA:
            return cavOctHfaGetPatternCount((CAV_OCT_HFA_MPSE *)p->obj);
#endif
    }
    return 0;
}
