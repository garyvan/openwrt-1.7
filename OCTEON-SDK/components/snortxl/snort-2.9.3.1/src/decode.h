...
/*  D E F I N E S  ************************************************************/
#ifdef CAV_OCT_HFA
#define CAV_OCT_ASYNC
/* SnortXL: Instead of BUGPRINTS use existing debug intf */
//#define BUGPRINTS
#endif
...
typedef enum{
    CAV_STATE_NONE, 
    CAV_STATE_SUBMIT,           /* Gets set if packet is submited to hfa search */
    CAV_STATE_PIPELINE,         /* Gets set for pipelined http request in the packet */
    CAV_STATE_FINAL,            /* Gets set where there cant be no more omds or search for the packet */
    CAV_STATE_CLEAN             /* Gets set when Packet is freed and evaluation is done */
}cav_State;
...
#ifdef CAV_OCT_ASYNC
	/* SnortXL: Modify placement of variables - this is a huge struct */

	/* state_flag signifies if packet has been submited for search or evaluation
	 * has been completed or packet has been freed
	 */
    cav_State state_flag;
	/* This counter represents the number of omds that can be associated with the
	 * packet - there are no references to the omds - instead omd contains
	 * reference to the packet
	 */
    uint8_t omd_count; 
    /* Added Matchinfo to packet to have a common match-array for multiple omds per packet */
    uint8_t *matchInfo;
#endif

    int iMatchInfoArraySize;
    /*Cycles that commence when the daq provides the packet to snort*/
    uint64_t snort_start_cycle;
    /*Cycles to adjust latency for multiple search*/
    uint64_t snort_diff_cycle;
    /*Cycles that commence when the snort start detecting the packet*/
    uint64_t detect_start_cycle;
    /*Cycles that commence when the snort starts preprocessing the packet*/
    uint64_t snort_preproc_cycle;
    /*Cycles that commence when the hfa start processing the packet*/
    uint64_t hfa_start_cycle;
    /*Cycles that commence when the pplib start processing the packet*/
    uint64_t pp_start_cycle;
