...
#ifdef CAV_OCT_HFA
    /* For rebuilt stream total dynamically allocated byte is 65536 */
    if ( type == PSEUDO_PKT_TCP )
        c->max_dsize = 65536 - (ETHERNET_HEADER_LEN + VLAN_HEADER_LEN + ETHERNET_MTU);
    else
#endif
...
#ifdef CAV_OCT_HFA
    // Cavium: HFA requires directly mapped memory 
    uint8_t* b = cavSnortAlloc(sizeof(*p->pkth) + PKT_MAX + SPARC_TWIDDLE);
#else
...
#ifdef CAV_OCT_ASYNC
    p->iMatchInfoArraySize = 8;
    p->matchInfo = (uint8_t *)SnortAlloc(sizeof(MATCH_INFO)*8); 
    if(!p->matchInfo)
        printf("match info allocation failed\n");
#endif
    p->pkth = (void*)b;
    b += sizeof(*p->pkth);
    b += SPARC_TWIDDLE;
...
#ifdef CAV_OCT_HFA
    // Cavium: HFA requires directly mapped memory 
    cavfree((void*)p->pkth,sizeof(*p->pkth) + PKT_MAX + SPARC_TWIDDLE);
#else
    free((void*)p->pkth);  // cast away const!
#ifdef CAV_OCT_ASYNC
    free((void*)p->matchInfo);
#endif
#endif
    free(p);
}
