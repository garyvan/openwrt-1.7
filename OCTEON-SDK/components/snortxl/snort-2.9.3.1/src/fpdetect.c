...
/* SnortXL Cavium: OMD per search group
 * Unlike in the case of Sync search or default search where one omd is
 * sufficient per snort instance, however in async case we need omd to remain
 * when we process the packet. Therefore apart from omd all data referenced
 * within omd like p, matchinfo etc should be preserved.
 *
 * In the absence of a pre-defined async architecture in snort, we are using
 * certain variables within omd 
 * 
 * submit_cnt - This flag signals the number of searches that are submitted for
 *              this omd - this is iincremented at each search submit and 
 *              decremented after each search result is processed
 * final_flag - This flag is set at a point after whihc there will be no further
 *              submits done to the current omd
 *
 * An omd can be free only if submit_cnt==0 and final_flag is set.
 *
 */

#define CAV_MAX_RULE_TYPES 8
uint64_t omd_acount=-1, omd_fcount=0; 
/* Function to fetch omd allocation stats */
...
#else
    OTNX_MATCH_DATA *omd = (OTNX_MATCH_DATA *)func_cav128BAlloc( );
#endif
    
    if(omd == NULL)
    {
        printf("Unable to allocate omd\n");
    }
...
#ifdef CAV_OCT_ASYNC
    if (omd->final_flag == 1 && omd->submit_cnt == 0 && !omd->pending)
#endif
    {

#ifdef CAV_OCT_ASYNC
		/* Packet will be tied to an omd only if there is search required
		 * There can bei multiple cases where there are no pkts tied to omd
		 * and the pkt requires no search 
		 * */
        if (omd->p)
            omd->p->omd_count--;
        omd_fcount++;
        ...
        free(omd);
#else
        // SnortXL: Edit this free - its obfuscated - what if the mebers are
        // reordered?
        func_cav128BFree((void*)omd) ;
#endif
#ifdef BUGPRINTS
        printf("[%s:%d] OMD Free %p scnt %d  C%d\n", omd, omd->submit_cnt, omd_fcount);
#endif
    }
}
...
static inline int fpFinalSelectEvent(OTNX_MATCH_DATA *o, Packet *p)
{
    ...
    for( i = 0; i < p->iMatchInfoArraySize; i++ )
    {
...
int cavfpFinalSelectEvent(OTNX_MATCH_DATA *o, Packet *p)
{
	return fpFinalSelectEvent (o, p);
}
#endif
...
# endif
...
#ifdef CAV_OCT_ASYNC
	/* SnortXL : Make constant - sc is huge struct 
	 * instead of 8 should be sc->num_rule_types */
    omd = OtnXMatchDataNew(8);
    omd->p = p;
			/* SnortXL: Increment the number of omds assigned to a packet */
    p->omd_count++;
#endif
...
#ifdef CAV_OCT_HFA
    omd->matchInfo = (MATCH_INFO *)p->matchInfo;
#endif
    if (do_detect_content)
    {
        ...
        {
            omd->pg = port_group;
			/* SnortXL: Here a pkt is assigned to an OMD */
            ...
            if (p->uri_count > 0)
            {
                ...
                for (i = HTTP_BUFFER_URI; (i < p->uri_count) && (i <= HTTP_BUFFER_CLIENT_BODY); i++)
                {
                    ...
                    if ((so != NULL) && (mpseGetPatternCount(so) > 0))
                    {
                        ...
                        /* SnortXL: These buffers are stack variables - will not
						 * be preserved - therefore copy it into another buffer
						 * for it to be availab;e duringsearch and post-process 
						 *
						 * Explore possibility of using original buffers in case
						 * there are no modifications/normalisations */
            ...
            if ((so != NULL) && (mpseGetPatternCount(so) > 0))
            {
                ...
                /* SnortXL: These buffers are stack variables - will not
				 * be preserved - therefore copy it into another buffer
				 * for it to be available during search and post-process
				 */
                ...
                if (p->data && p->dsize)
                {
                    ...
                    start_state = 0;
                    /* 
					 * These are proper buffers and will stay till pkt is freed.
					 * modify  acordingly for perf improvement - avg  800 cycles 
					 * per search through  this call can be avoiding copy
					 *
					 * */ 
...
#ifdef CAV_OCT_ASYNC
	/* No search submits for this omd beyond this point 
	 * This omd can be freed now if there are no pending 
	 * associated searches to be handled.
	 * */
    omd->final_flag = 1;
...
#ifdef CAV_OCT_ASYNC
    omd->matchInfo = (MATCH_INFO *)p->matchInfo;
    if(p->state_flag == CAV_STATE_SUBMIT)
        return 0;
#endif
