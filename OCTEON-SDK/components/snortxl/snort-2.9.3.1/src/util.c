...
#ifdef CAV_OCT_HFA
// Cavium: to include cavium specific definitions 
#include "misc_defs.h"
#include "cav_oct_hfa.h"
extern int cavOctDaqFlag;
extern int cavOctPcapFlag;
#define cav_oct_core cvmx_get_core_num()
void getStats(uint64_t *, uint64_t *);
#endif /* CAV_OCT_HFA */

#ifdef CAV_OCT_SE
// Cavium: All generic unistd.h defs are not supported in SE 
#include <custom/unistd.h>
#endif
...
#ifdef CAV_OCT_HFA
// Cavium: support for single core stats printing 
CVMX_SHARED static DAQ_Stats_t cav_daq_stats[CVMX_MAX_CORES+1];
CVMX_SHARED static PacketCount cav_pc[CVMX_MAX_CORES +1];
CVMX_SHARED double cav_total_secs[CVMX_MAX_CORES+1];
#ifdef ACTIVE_RESPONSE
CVMX_SHARED uint64_t cav_pkts_inj[CVMX_MAX_CORES+1];
#endif

void setTotalValues_cav_daq_stats()
{
    if (cavlast_core())
    {   
        memset(&cav_daq_stats[CVMX_MAX_CORES],0,sizeof(DAQ_Stats_t));
        int i = 0, j = 0;
        for(; i < CVMX_MAX_CORES; i++)
        {
            cav_daq_stats[CVMX_MAX_CORES].hw_packets_received +=  cav_daq_stats[i].hw_packets_received;
            cav_daq_stats[CVMX_MAX_CORES].hw_packets_dropped  +=  cav_daq_stats[i].hw_packets_dropped;
            cav_daq_stats[CVMX_MAX_CORES].packets_received    +=  cav_daq_stats[i].packets_received;
            cav_daq_stats[CVMX_MAX_CORES].packets_filtered    +=  cav_daq_stats[i].packets_filtered;
            cav_daq_stats[CVMX_MAX_CORES].packets_injected    +=  cav_daq_stats[i].packets_injected;
            for(j = 0; j < MAX_DAQ_VERDICT; j++)
                cav_daq_stats[CVMX_MAX_CORES].verdicts[j]     +=  cav_daq_stats[i].verdicts[j];
        }
    }
}

void setTotalValues_cav_pc()
{
    if (cavlast_core())
    {
        memset(&cav_pc[CVMX_MAX_CORES],0,sizeof(PacketCount));
        int i=0;
        for(;i < CVMX_MAX_CORES;i++)
        {
           cav_pc[CVMX_MAX_CORES].total_from_daq   +=   cav_pc[i].total_from_daq;
           cav_pc[CVMX_MAX_CORES].total_processed  +=   cav_pc[i].total_processed;
                                                                                              
           cav_pc[CVMX_MAX_CORES].s5tcp1           +=   cav_pc[i].s5tcp1;
           cav_pc[CVMX_MAX_CORES].s5tcp2           +=   cav_pc[i].s5tcp2;
           cav_pc[CVMX_MAX_CORES].ipv6opts         +=   cav_pc[i].ipv6opts;
           cav_pc[CVMX_MAX_CORES].eth              +=   cav_pc[i].eth;
           cav_pc[CVMX_MAX_CORES].ethdisc          +=   cav_pc[i].ethdisc;
           cav_pc[CVMX_MAX_CORES].ipv6disc         +=   cav_pc[i].ipv6disc;
           cav_pc[CVMX_MAX_CORES].ip6ext           +=   cav_pc[i].ip6ext;
           cav_pc[CVMX_MAX_CORES].other            +=   cav_pc[i].other;
           cav_pc[CVMX_MAX_CORES].tcp              +=   cav_pc[i].tcp;
           cav_pc[CVMX_MAX_CORES].udp              +=   cav_pc[i].udp;
           cav_pc[CVMX_MAX_CORES].icmp             +=   cav_pc[i].icmp;
           cav_pc[CVMX_MAX_CORES].arp              +=   cav_pc[i].arp;
#ifndef NON_ETHER_DECODER
           cav_pc[CVMX_MAX_CORES].eapol            +=   cav_pc[i].eapol;
#endif
           cav_pc[CVMX_MAX_CORES].vlan             +=   cav_pc[i].vlan;
           cav_pc[CVMX_MAX_CORES].nested_vlan      +=   cav_pc[i].nested_vlan;
           cav_pc[CVMX_MAX_CORES].ipv6             +=   cav_pc[i].ipv6;
           cav_pc[CVMX_MAX_CORES].ipv6_up          +=   cav_pc[i].ipv6_up;
           cav_pc[CVMX_MAX_CORES].ipv6_upfail      +=   cav_pc[i].ipv6_upfail;
           cav_pc[CVMX_MAX_CORES].frag6            +=   cav_pc[i].frag6;
           cav_pc[CVMX_MAX_CORES].icmp6            +=   cav_pc[i].icmp6;
           cav_pc[CVMX_MAX_CORES].tdisc            +=   cav_pc[i].tdisc;
           cav_pc[CVMX_MAX_CORES].udisc            +=   cav_pc[i].udisc;
           cav_pc[CVMX_MAX_CORES].tcp6             +=   cav_pc[i].tcp6;
           cav_pc[CVMX_MAX_CORES].udp6             +=   cav_pc[i].udp6;
           cav_pc[CVMX_MAX_CORES].teredo           +=   cav_pc[i].teredo;
           cav_pc[CVMX_MAX_CORES].ipdisc           +=   cav_pc[i].ipdisc;
           cav_pc[CVMX_MAX_CORES].icmpdisc         +=   cav_pc[i].icmpdisc;
           cav_pc[CVMX_MAX_CORES].embdip           +=   cav_pc[i].embdip;
           cav_pc[CVMX_MAX_CORES].ip               +=   cav_pc[i].ip;
           cav_pc[CVMX_MAX_CORES].ipx              +=   cav_pc[i].ipx;
           cav_pc[CVMX_MAX_CORES].ethloopback      +=   cav_pc[i].ethloopback;

           cav_pc[CVMX_MAX_CORES].invalid_checksums +=   cav_pc[i].invalid_checksums;
           cav_pc[CVMX_MAX_CORES].bad_ttl           +=   cav_pc[i].bad_ttl;
#ifdef GRE                                                
           cav_pc[CVMX_MAX_CORES].ip4ip4            +=   cav_pc[i].ip4ip4;
           cav_pc[CVMX_MAX_CORES].ip4ip6            +=   cav_pc[i].ip4ip6;
           cav_pc[CVMX_MAX_CORES].ip6ip4            +=   cav_pc[i].ip6ip4;
           cav_pc[CVMX_MAX_CORES].ip6ip6            +=   cav_pc[i].ip6ip6;
                                                                                               
           cav_pc[CVMX_MAX_CORES].gre               +=   cav_pc[i].gre;
           cav_pc[CVMX_MAX_CORES].gre_ip            +=   cav_pc[i].gre_ip;
           cav_pc[CVMX_MAX_CORES].gre_eth           +=   cav_pc[i].gre_eth;
           cav_pc[CVMX_MAX_CORES].gre_arp           +=   cav_pc[i].gre_arp;
           cav_pc[CVMX_MAX_CORES].gre_ipv6          +=   cav_pc[i].gre_ipv6;
           cav_pc[CVMX_MAX_CORES].gre_ipv6ext       +=   cav_pc[i].gre_ipv6ext;
           cav_pc[CVMX_MAX_CORES].gre_ipx           +=   cav_pc[i].gre_ipx;
           cav_pc[CVMX_MAX_CORES].gre_loopback      +=   cav_pc[i].gre_loopback;
           cav_pc[CVMX_MAX_CORES].gre_vlan          +=   cav_pc[i].gre_vlan;
           cav_pc[CVMX_MAX_CORES].gre_ppp           +=   cav_pc[i].gre_ppp;
#endif                                                    
                                                                                               
           cav_pc[CVMX_MAX_CORES].discards          +=   cav_pc[i].discards;
           cav_pc[CVMX_MAX_CORES].alert_pkts        +=   cav_pc[i].alert_pkts;
           cav_pc[CVMX_MAX_CORES].log_pkts          +=   cav_pc[i].log_pkts;
           cav_pc[CVMX_MAX_CORES].pass_pkts         +=   cav_pc[i].pass_pkts;
                                                                               
           cav_pc[CVMX_MAX_CORES].match_limit       +=   cav_pc[i].match_limit;
           cav_pc[CVMX_MAX_CORES].queue_limit       +=   cav_pc[i].queue_limit;
           cav_pc[CVMX_MAX_CORES].log_limit         +=   cav_pc[i].log_limit;
           cav_pc[CVMX_MAX_CORES].event_limit       +=   cav_pc[i].event_limit;
           cav_pc[CVMX_MAX_CORES].alert_limit       +=   cav_pc[i].alert_limit;
                                                                                               
           cav_pc[CVMX_MAX_CORES].frags             +=   cav_pc[i].frags;
           cav_pc[CVMX_MAX_CORES].frag_trackers     +=   cav_pc[i].frag_trackers;
           cav_pc[CVMX_MAX_CORES].rebuilt_frags     +=   cav_pc[i].rebuilt_frags;
           cav_pc[CVMX_MAX_CORES].frag_incomp       +=   cav_pc[i].frag_incomp;
           cav_pc[CVMX_MAX_CORES].frag_timeout      +=   cav_pc[i].frag_timeout;
           cav_pc[CVMX_MAX_CORES].rebuild_element   +=   cav_pc[i].rebuild_element;
           cav_pc[CVMX_MAX_CORES].frag_mem_faults   +=   cav_pc[i].frag_mem_faults;
                                                                                               
           cav_pc[CVMX_MAX_CORES].tcp_stream_pkts   +=   cav_pc[i].tcp_stream_pkts;
           cav_pc[CVMX_MAX_CORES].rebuilt_tcp       +=   cav_pc[i].rebuilt_tcp;
           cav_pc[CVMX_MAX_CORES].tcp_streams       +=   cav_pc[i].tcp_streams;
           cav_pc[CVMX_MAX_CORES].rebuilt_segs      +=   cav_pc[i].rebuilt_segs;
           cav_pc[CVMX_MAX_CORES].queued_segs       +=   cav_pc[i].queued_segs;
           cav_pc[CVMX_MAX_CORES].str_mem_faults    +=   cav_pc[i].str_mem_faults;
                                                                                              
#ifdef TARGET_BASED
           cav_pc[CVMX_MAX_CORES].attribute_table_reloads +=   cav_pc[i].attribute_table_reloads;
#endif
                                                                                              
#ifndef NO_NON_ETHER_DECODERR
#ifdef DLT_IEEE802_11
   /* wireless statistics */
           cav_pc[CVMX_MAX_CORES].wifi_mgmt         +=   cav_pc[i].wifi_mgmt;
           cav_pc[CVMX_MAX_CORES].wifi_data         +=   cav_pc[i].wifi_data;
           cav_pc[CVMX_MAX_CORES].wifi_control      +=   cav_pc[i].wifi_control;
           cav_pc[CVMX_MAX_CORES].assoc_req         +=   cav_pc[i].assoc_req;
           cav_pc[CVMX_MAX_CORES].assoc_resp        +=   cav_pc[i].assoc_resp;
           cav_pc[CVMX_MAX_CORES].reassoc_req       +=   cav_pc[i].reassoc_req;
           cav_pc[CVMX_MAX_CORES].reassoc_resp      +=   cav_pc[i].reassoc_resp;
           cav_pc[CVMX_MAX_CORES].probe_req         +=   cav_pc[i].probe_req;
           cav_pc[CVMX_MAX_CORES].probe_resp        +=   cav_pc[i].probe_resp;
           cav_pc[CVMX_MAX_CORES].beacon            +=   cav_pc[i].beacon;
           cav_pc[CVMX_MAX_CORES].atim              +=   cav_pc[i].atim;
           cav_pc[CVMX_MAX_CORES].dissassoc         +=   cav_pc[i].dissassoc;
           cav_pc[CVMX_MAX_CORES].auth              +=   cav_pc[i].auth;
           cav_pc[CVMX_MAX_CORES].deauth            +=   cav_pc[i].deauth;
           cav_pc[CVMX_MAX_CORES].ps_poll           +=   cav_pc[i].ps_poll;
           cav_pc[CVMX_MAX_CORES].rts               +=   cav_pc[i].rts;
           cav_pc[CVMX_MAX_CORES].cts               +=   cav_pc[i].cts;
           cav_pc[CVMX_MAX_CORES].ack               +=   cav_pc[i].ack;
           cav_pc[CVMX_MAX_CORES].cf_end            +=   cav_pc[i].cf_end;
           cav_pc[CVMX_MAX_CORES].cf_end_cf_ack     +=   cav_pc[i].cf_end_cf_ack;
           cav_pc[CVMX_MAX_CORES].data              +=   cav_pc[i].data;
           cav_pc[CVMX_MAX_CORES].data_cf_ack       +=   cav_pc[i].data_cf_ack;
           cav_pc[CVMX_MAX_CORES].data_cf_poll      +=   cav_pc[i].data_cf_poll;
           cav_pc[CVMX_MAX_CORES].data_cf_ack_cf_poll +=   cav_pc[i].data_cf_ack_cf_poll;
           cav_pc[CVMX_MAX_CORES].cf_ack              +=   cav_pc[i].cf_ack;
           cav_pc[CVMX_MAX_CORES].cf_poll             +=   cav_pc[i].cf_poll;
           cav_pc[CVMX_MAX_CORES].cf_ack_cf_poll      +=   cav_pc[i].cf_ack_cf_poll;
#endif
#endif  // NO_NON_ETHER_DECODER
                                                                                              
#ifdef MPLS                                            
           cav_pc[CVMX_MAX_CORES].mpls                +=   cav_pc[i].mpls;
#endif
       }
    }
}
...
void ErrorMessage(const char *format,...)
{
    ...
    {
#ifdef CAV_OCT_HFA
        /* Cavium: error directed to console */
        vsnprintf(buf, STD_BUF, format, ap);
        buf[STD_BUF] = '\0';
        printf(buf);
#else
...
NORETURN void FatalError(const char *format,...)
{
    ...
    {
#ifdef CAV_OCT_HFA
        // Cavium: call exit routine of HFA in case of error 
    ...
    {
        DAQ_BreakLoop(1);
#if !defined(WIN32) && !defined(CAV_OCT_HFA)
        // Cavium: pthread not supported 
        pthread_exit(NULL);
#endif
    }
}
...
void TimeStart (void)
{
    ...
    if(cavlast_core())
    {
      memset(cav_pc,0,sizeof(cav_pc));
      memset(cav_daq_stats,0,sizeof(cav_daq_stats));
    }
    cavbarrier_sync();
#endif /* CAV_OCT_HFA */
...
/* Bypassing stats for compilation as they are not needed for Graph compilation */
#ifdef CAV_OCT_HFA
    // Cavium: support for single core stats printing 
    uint64_t divider = 1, usec_tmp = 0;
    double avg_secs_per_core= 0.0;
#ifdef DEBUG
    printf("%u Run time for packet processing was %lu.%lu seconds\n",cav_oct_core,
        (unsigned long)difftime.tv_sec, (unsigned long)difftime.tv_usec);
#endif
    cav_pc[cav_oct_core] = pc;
    usec_tmp = difftime.tv_usec;
    while(usec_tmp){
        usec_tmp /= 10;
        divider *= 10;
    }

    cav_total_secs[cav_oct_core] = difftime.tv_sec + ((double)difftime.tv_usec/divider) ;
    cavbarrier_sync();
    if (cavlast_core())
    {   
      int i = 0;
      cav_total_secs[CVMX_MAX_CORES] = 0;
      for(; i < CVMX_MAX_CORES; i++)
      {
        cav_total_secs[CVMX_MAX_CORES] += cav_total_secs[i];
#ifdef DEBUG
        printf("cav_total_secs[%d]=%lf\n",i,cav_total_secs[i]);
#endif
      }
      setTotalValues_cav_pc();
      avg_secs_per_core = cav_total_secs[CVMX_MAX_CORES]/cav_coremask();
      LogMessage("Run time for packet processing was %lf seconds\n",avg_secs_per_core);

#define pc cav_pc[CVMX_MAX_CORES]
#else
...
#ifdef CAV_OCT_HFA
    // Cavium: support for single core stats printing 
    tmp = total_secs = (uint32_t)avg_secs_per_core;
#else
    ...
    pps = (pc.total_from_daq / avg_secs_per_core);
    LogCount("Pkts/sec", pps);
#ifdef CAV_OCT_HFA
   }
#endif
#endif /* CAV_OCT_HFA_GCOMPILE */
}
...
#undef pc
#endif
    cav_daq_stats[cav_oct_core] = *pkt_stats;
    cav_pc[cav_oct_core] = pc;
    cavbarrier_sync();
    if (cavlast_core())
    {
        setTotalValues_cav_daq_stats();
        setTotalValues_cav_pc();
    }
    uint64_t total = cav_pc[CVMX_MAX_CORES].total_processed;
    uint64_t pkts_recv = cav_pc[CVMX_MAX_CORES].total_from_daq;
#endif /* CAV_OCT_HFA */

#if !defined(CAV_OCT_HFA_GCOMPILE)
#ifdef PPM_MGR
    PPM_PRINT_SUMMARY(&snort_conf->ppm_cfg);
#endif
    {
        uint64_t pkts_drop, pkts_out, pkts_inj = 0;
...
#ifdef ACTIVE_RESPONSE
        int i=0;
        cav_pkts_inj[cav_oct_core] = Active_GetInjects() ;
        cavbarrier_sync();
        if (cavlast_core())
          for(;i < CVMX_MAX_CORES;i++)
             cav_pkts_inj[CVMX_MAX_CORES] += cav_pkts_inj[i];
 
#endif

        pkts_recv = cav_daq_stats[CVMX_MAX_CORES].hw_packets_received;
        pkts_drop = cav_daq_stats[CVMX_MAX_CORES].hw_packets_dropped;

        pkts_out = pkts_recv - cav_daq_stats[CVMX_MAX_CORES].packets_filtered
                             - cav_daq_stats[CVMX_MAX_CORES].packets_received;

        pkts_inj = cav_daq_stats[CVMX_MAX_CORES].packets_injected;
#ifdef ACTIVE_RESPONSE
        pkts_inj += cav_pkts_inj[CVMX_MAX_CORES];
#endif
        if(cavlast_core())
        {
#endif /* CAV_OCT_HFA */
...
#else
            if(cavOctPcapFlag)
            LogStat("Analyzed", cav_daq_stats[CVMX_MAX_CORES].packets_received, pkts_recv);
            else
            LogStat("Analyzed",cav_pc[CVMX_MAX_CORES].total_from_daq, pkts_recv);
            LogStat("Dropped", pkts_drop, pkts_recv+pkts_drop);
            LogStat("Filtered", cav_daq_stats[CVMX_MAX_CORES].packets_filtered,pkts_recv);
#endif /* CAV_OCT_HFA */
            if(cavOctPcapFlag)  //if daq is pcap 
            LogStat("Outstanding", pkts_recv-pkts_drop-cav_pc[CVMX_MAX_CORES].total_from_daq, pkts_recv);
            else
    ...
    // Bypassing stats as it's not needed in case of graph generation 
#ifdef CAV_OCT_HFA
    // Cavium: support for single core stats printing 
#define pc cav_pc[CVMX_MAX_CORES]
    if(cavlast_core())
...
#else
            LogStat(s, cav_daq_stats[CVMX_MAX_CORES].verdicts[i], pkts_recv);
#endif
...
void GoDaemon(void)
{
    ...
    int exit_val = 0;
    // Cavium: prevent compilation warnings
    pid_t cpid = 0;
...
#endif
#ifdef CAV_OCT_SE
                // Cavium: sleep fn equivalent in SE 
                cav_wait(1500); /* replace bu CPU_CLK for 1sec */
#else
                sleep(1);
#endif
    ...
    open("/tmp/snort.debug", O_CREAT | O_RDWR,0666);  /* stdin, fd 0 */
    ...
    (void)open("/dev/null", O_RDWR, 0666);  /* stdin, fd 0 */
#endif
#ifndef CAV_OCT_SE
    // Cavium: not supported 
...
SF_LIST * SortDirectory(const char *path)
{
    ...
    dir_entries = NULL;
#ifndef CAV_OCT_SE
    // Cavium: directories not supported in SE
    DIR *dir;
...
{
#ifndef CAV_OCT_SE
    // Cavium: Not supported as passthrough is the only network interface in SE
    struct ifreq ifr;
