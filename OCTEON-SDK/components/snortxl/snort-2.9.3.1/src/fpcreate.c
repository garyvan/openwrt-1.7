...
#endif
#if defined CAV_OCT_HFA || CAV_OCT_HFA_GCOMPILE
    // Cavium: Add HFA as search engine
    // SnortXL : Revisit - too messy, no need of so many variables 
    else if( !strcasecmp(method,"cav_oct_hfa") )
    {
       search_type = ASYNC;
       fp->search_method = MPSE_CAV_OCT_HFA;
#ifdef CAV_OCT_HFA
       cav_search_method = CAV_OCT_HFA_ENGINE;
       cavOctHfaInit();
#endif
       LogMessage("   Search-Method = Cavium OCTEON HFA\n");
    }
    else if( !strcasecmp(method,"cav_oct_hfa_sync") )
    {
       search_type = SYNC;
       fp->search_method = MPSE_CAV_OCT_HFA;
#ifdef CAV_OCT_HFA
       cav_search_method = CAV_OCT_HFA_ENGINE;
	   /* SnortXL : Set pipeline depth here? */
       cavOctHfaInit();
#endif
       LogMessage("   Search-Method = Cavium OCTEON HFA SYNC\n");
    }
#endif
    else
    {
...
static int fpCreatePortGroups(SnortConfig *sc, rule_port_tables_t *p)
{
...
#ifdef CAV_OCT_LINUX
    // Cavium: support for graph generation 
    cavgraphpath();
#endif
    ...
    cavOctHfaExit();
#endif
    LogMessage("SnortXL Rule Compiler Exiting\n");
    DAQ_Abort();
    exit(0);
#endif
	return 0;
