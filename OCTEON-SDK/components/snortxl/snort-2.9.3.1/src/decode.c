...
//    p->ip_payload_off = p->ip_payload_len + (int)pkt;
    ...
    if (ScIpChecksums())
    {
		...
		if (cavOctDaqFlag)
		{
			if (cav_oct_err_flag == CAV_ERR_IP_CS) 
			    csum =  CAV_ERR_IP_CS;
		}
        else
#endif
...
void DecodeTCP(const uint8_t * pkt, const uint32_t len, Packet * p)
{
    ...
    if (ScTcpChecksums())
	{
		...
		if (cavOctDaqFlag)
		{
			/* Use TCP checksum calculated by PIP dont do in sw */
			if (cav_oct_err_flag == CAV_ERR_L4_CS) 
				csum =  CAV_ERR_L4_CS;
		}
		else
#endif
    ...
    if(cap_len < PFLOG2_HDRMIN)
    {
        if (ScLogVerbose())
        {
            ErrorMessage("Captured data length < minimum Pflog length! "
                    "(%d < %llu)\n", cap_len, (unsigned long long)PFLOG2_HDRMIN);
        }
        PREPROC_PROFILE_END(decodePerfStats);
        return;
    }
